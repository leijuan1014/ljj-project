define(['jquery', "components", "common", "weui"], function (jquery, components, common, weui) {
    ...
    function payByWX() {
        $.post(apiUrl + "/wxpay/pay", {
            "orderNumber": orderNumber,
            "openid":"oFxaJw5Ejp1DfwZZEzUUzDZAHIRU" //TODO
        }, function (data) {
            data = JSON.parse(data);
            /*data = {
              "return_code" : true,
              "orderId" : "S201707261500003",
              "codeUrl" : null,
              "appId" : "wx46b543acf6dbc7fa",
              "timeStamp" : "1501113479",
              "nonceStr" : "BA018F553FC1E9A858EC70443C9F7A56",
              "packageStr" : "prepay_id=wx20170726165635df032387840925488450",
              "signType" : "MD5",
              "sign" : "D1C48F949326567B43E965B0ABA84FBC"
            };
            */
            if (data.return_code == false) {
                $.toast("下单失败", "text");
                return;
            }
            ...
            prepay_id = data.packageStr.toString();
            appId = data.appId.toString();
            timeStamp = data.timeStamp.toString();
            //alert("paySign="+paySign+",nonceStr=" +nonceStr + ",prepay_id="+prepay_id + ",appId="+appId + ",timeStamp=" + timeStamp );
            if (typeof WeixinJSBridge == "undefined") {
                if (document.addEventListener) {
                    document.addEventListener('WeixinJSBridgeReady', onBridgeReady, false);
                } else if (document.attachEvent) {
