define(['jquery', "template", "components", "ZeroClipboard", "jqueryValidate", "ueditorConfig", "ueditorAll", "manageCommon"], function (jquery, template, components, ZeroClipboard, validator, ueditorConfig, UE, manageCommon) {
        ...
        components.getMsg(url).done(function (msg) {
            ...
            if (res == 1) {
                ...
                if (msg.obj.image != "") {
                    ...
                    $('#pic-document-list').append($('<li picUrl="' + arr + '"class="apply-book"> <img  src=" ' + imgUrl + '">  <a docUrl="' + arr + '" href="javascript:void(0)" class="text-basic del-doc"><i class="iconfont icon-delete"></i></a></li>'));
                }
            }
        });
    }
    ...
    function upPic() {
      /*
      $("#pic-upload").fileupload({
          url: apiUrl + "/upload/uploadimg/dydz/user",
          type:"POST",
          dataType:"json",
          add: function (e, data) {
            ...
            alert(msg.result.imageUrl);
            var picListHtml = '<li picUrl="' + msg.result.imageUrl + '"class="apply-book"> <img  src=" ' + apiUrlPic + msg.result.imageUrl + '">  <a docUrl="' + msg.result.imageUrl + '" href="javascript:void(0)" class="text-basic del-doc"><i class="iconfont icon-delete"></i></a></li>';
            $("#pic-document-list").append(picListHtml);
            $(".a-upload").css("display", "none");
          },
          fail:function(e, data){
              alert(JSON.stringify(result.result.messages));
          }
      });
      */
        ...
        $("#div-box").on("change", "#pic-upload", function () {
            ...
            $.ajax({
                ...
                success: function (msg) {
                    var picListHtml = '<li picUrl="' + msg.imageUrl + '"class="apply-book"> <img  src=" ' + apiUrlPic + msg.imageUrl + '">  <a docUrl="' + msg.imageUrl + '" href="javascript:void(0)" class="text-basic del-doc"><i class="iconfont icon-delete"></i></a></li>';
                    $("#pic-document-list").append(picListHtml);
                    $(".a-upload").css("display", "none");
                }
            });
