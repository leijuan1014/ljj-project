...
public class OrderAdminController extends AbstractController {
	...
	public JsonResponse<PageResult<Order>> getOrderByPage(
		...
		orderItemService.queryByPage(page, order);
		if (page.getTotal() != 0) {
	...
	public JsonResponse<Order> getOrderByOrderNumber(String orderNumber,
		...
		Order order = orderItemService.getOne(orderNumber);
		if (order != null) {
			result.setRes(SystemCode.SUCCESS);
			result.setObj(order);
		}
	...
	public JsonResponse<String> toSend(Order order, HttpServletRequest request) {
		...
		order = orderItemService.selectByPrimaryKey(order.getOrderId());
		try {
	...
	public JsonResponse<String> receiveMoney(Order order,
		...
		try {
			...
			if (order.getLogisticsState() == 3 && order.getPayState() != 2) {
				...
				orderItemService.updateAndAddCertify(order, orderCertify);
	...
	public JsonResponse<List<Object>> getOrderByPaystate(
		...
		try {
			...
			// 未付款金额
			Integer paid = orderItemService.getTotalAmountByPaystate(1);
			map.put("name", "未支付订单");
			map.put("y", paid);
			dataList.add(map);
			// 已付款
			Map<String, Object> map2 = new HashMap<String, Object>();
			Integer unPaid = orderItemService.getTotalAmountByPaystate(2);
			map2.put("name", "已支付订单");
			map2.put("y", unPaid);
			dataList.add(map2);
	...
	public JsonResponse<List<Object>> getCountByPaystate(
		...
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			// 未付款金额
			Integer paidNum = orderItemService.getCountByPaystate(1);
			map.put("name", "未支付订单");
			map.put("y", paidNum);
			dataList.add(map);
			// 已付款
			Map<String, Object> map2 = new HashMap<String, Object>();
			Integer unPaidNum = orderItemService.getCountByPaystate(2);
			map2.put("name", "已支付订单");
	...
	public JsonResponse<PageResult<Order>> getOrderPageByOrderAdminSearchVO(
		...
		order.setCheckAll("1");
		try {
			orderItemService.getPageByOrderAdminSearchVO(page, order);
		} catch (Exception e) {
			result.setRes(SystemCode.FAILURE);
	...
	public void exportPageByOrderAdminSearchVO(OrderAdminSearchVO order,
		...
		try {
			List<Order> result = orderItemService
