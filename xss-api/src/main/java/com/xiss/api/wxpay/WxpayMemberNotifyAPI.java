...
public class WxpayMemberNotifyAPI{
		...
		if (attach != null && !attach.isEmpty()) {
			...
			if (licensedId != null && !licensedId.isEmpty()) {
				...
				if (car != null) {
					...
					Users users = usersService.getById(car.getUserId());
					if(users.getRoles() == RolesEnum.REGISTED.ordinal())
						usersService.updateRoles(users.getMobile(), RolesEnum.MEMBER.ordinal());
					
					JavaSmsApi sms = new JavaSmsApi();
					
					try {
						sms.sendMemberMsg(users.getMobile(), licensedId, "年", valid);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				System.out.println("==========after.car=" + car);
			}
		}
	}
